...
from pre_b import (
    ...
    add_is_first_test_feature_b,
    add_holiday_season_feature,
)
from pk_dict_utils import pp_concat_user_core_features, load_pk_dict
...
if __name__ == "__main__":
    ...
    # 첫 시험 여부 피처 추가
    print("➕ 첫 시험 여부 피처 추가 중...")
    dfB = add_is_first_test_feature_b(dfB, label_pattern_history_b, out_col='is_first_test')
    
    # 연휴 여부 피처 추가 (6개월 이내 9월/10월 포함 여부)
    print("➕ 연휴 여부 피처 추가 중 (6개월 이내 9/10월)...")
    dfB = add_holiday_season_feature(dfB, out_col='has_holiday_season', months_ahead=6)
    
    # =========================================================
    # 🔸 pk_dict 생성 및 저장 (A 데이터 기반 시계열 피처용)
    # =========================================================
    print("\n🔑 pk_dict 생성 중 (A 데이터 시계열 이력)...")
    pk_dict = {}
    for pk, group in dfA.groupby('PrimaryKey'):
        records = []
        for _, row in group.iterrows():
            test_date = row['TestDate']
            label = int(row['Label'])
            records.append((test_date, label))
        pk_dict[pk] = sorted(records, key=lambda x: x[0])
    
    # pk_dict 저장
    pk_dict_path = 'pk_dict.pkl'
    with open(pk_dict_path, 'wb') as f:
        pickle.dump(pk_dict, f)
    print(f"✅ pk_dict 생성 및 저장 완료: {len(pk_dict)} keys → {pk_dict_path}")
    
    # =========================================================
    # 🔸 pk_dict 사용하여 user core features 추가
    # =========================================================
    print("\n🔧 User core features 추가 중 (pk_dict 기반)...")
    dfB = pp_concat_user_core_features(dfB, pk_dict)
    print(f"✅ User core features 추가 완료!")
    ...
    # pk_dict에서 추가된 A_ 피처들도 전달 (동적으로 처리)
    a_feature_cols = [col for col in dfB.columns if col.startswith('A_') or col.startswith('Age_')]
    for col in a_feature_cols:
        if col not in dfB_processed.columns:
            dfB_processed[col] = dfB[col].values
    
    if len(a_feature_cols) > 0:
        print(f"   ✅ pk_dict A_ 피처 추가: {len(a_feature_cols)}개 - {a_feature_cols[:5]}...")
    ...
    out_path = "models"
    # Save model
    out_path = out_path + f"/cat_jae_{score:.6f}.cbm"
    ...
    # 🔸 Feature 이름 저장
    # =========================================================
    feature_names_path = "models/feature_names_jae.json"
    feature_payload = {
        "features": meta_data,
    ...
    # =========================================================
    print(f"\n{'='*60}")
    print(f"📦 Test 추론 시 필요한 파일들 (백업 버전 - pk_dict 사용):")
    print(f"{'='*60}")
    print(f"1. {preprocessor_path} (전처리 객체 - 모든 PCA 포함)")
    print(f"2. {dict_A_path} (A 과거 이력)")
    print(f"3. {out_path} (학습된 CatBoost 모델)")
    print(f"4. {pk_dict_path} (A 데이터 시계열 이력 - user core features용)")
    print(f"5. models/label_pattern_history_b.json (B 패턴 이력)")
    print(f"6. models/label_pattern_history_a.json (A 패턴 이력)")
    print(f"7. models/primarykey_testdate_dict_b.json (B TestDate dict)")
    print(f"8. {feature_names_path} (Feature 이름)")
    print(f"9. optimal_preprocess_full.py (전처리 코드)")
    print(f"10. pk_dict_utils.py (pk_dict 처리 함수)")
    print(f"{'='*60}")
    print(f"\n💡 생성된 A_ 피처: {len([c for c in meta_data if c.startswith('A_')])}개")
    print(f"   예시: {[c for c in meta_data if c.startswith('A_')][:5]}")
    print(f"{'='*60}")
